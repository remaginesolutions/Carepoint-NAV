report 16035417 "NDIS Sessions List"
{
    // version CAREPOINT1.00

    // CAREPOINT1.00 2017-01-01: Object created
    DefaultLayout = RDLC;
    RDLCLayout = './NDIS Sessions List.rdlc';

    Caption = 'NDIS Sessions List';
    ...
    dataset
    {
        dataitem(DataItem8019; "Job")
        {
            ...
            dataitem(DataItem5444; "Integer")
            {
                ...
                column(JobTaskCaption; DataItem2969.TABLECAPTION + ': ' + JTFilter)
                {
                }
                column(ShowJTFilter; JTFilter)
                {
                }
                column(Desc_Job; DataItem8019.Description)
                {
                }
                column(CurrCodeJob0Fld; JobCalcBatches.GetCurrencyCode(DataItem8019, 0, CurrencyField))
                {
                }
                column(CurrCodeJob2Fld; JobCalcBatches.GetCurrencyCode(DataItem8019, 2, CurrencyField))
                {
                }
                column(CurrCodeJob3Fld; JobCalcBatches.GetCurrencyCode(DataItem8019, 3, CurrencyField))
                {
                }
            ...
            dataitem(DataItem2969; "Job Task")
            {
                ...
                dataitem(DataItem9714; "Job Planning Line")
                {
                    DataItemLink = "Job No." = FIELD ("Job No."),
                                   "Job Task No." = FIELD ("Job Task No."),
                                   "Planning Date" = FIELD ("Planning Date Filter");
                    DataItemLinkReference = DataItem2969;
    ...
    trigger OnPreReport()
    begin
        JTFilter := DataItem2969.GETFILTERS;
        JobFilter := DataItem2969.GETFILTER("Job No.");
        JobPlanningDateFilter := DataItem2969.GETFILTER("Planning Date Filter");
    end;
